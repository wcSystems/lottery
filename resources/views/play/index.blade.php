...
@section('content')
    <ol class="breadcrumb page-breadcrumb">
        ...
        <li class="breadcrumb-item">Plays</li>
    </ol>
                ...
                <div class="panel-content">
                    <div class="row">
                        <div class="form-group col-sm-4 mb-15">
                            <label class="form-label" for="loteria">Loterias</label>
                            <select name="loteria" type="text" class="@error("loteria") is-invalid @enderror form-control" value="" required>
                                <option selected value="none" disabled>Elija un opcion</option>
                                @foreach($lotteries as $sec)
                                    <option value="{{$sec->id}}">{{$sec->name}}</option>
                                @endforeach
                            </select>
                            {{$box}}
                            @error("loteria") <div class="invalid-feedback">Este campo es obligatorio</div> @enderror
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <table class="table table-striped table-hover">
                        ...
                        @foreach($lotteries as $item)
                            <tr>
                                ...
                                <td class="center"><a class="btn btn-secondary" href="/lotteries/{{$item->id}}"><i class="fa fa-edit"></i></a></td>
                                ...
                                <div class="modal-dialog modal-dialog-centered" role="document">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-footer">
                                            ...
                                            <a type="button" href="/lotteries/{{$item->id}}/remove" class="btn btn-primary">Continuar</a>
                                        </div>
                                    </div>
                                </div>
                            </div>
